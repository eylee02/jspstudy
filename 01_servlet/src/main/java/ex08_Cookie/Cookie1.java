...
public class Cookie1 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Cookie cookie3 = new Cookie("address",  URLEncoder.encode("서울특별시 종로구", "UTF-8"));  // 공백은 유효하지 않은 문자열이므로 인코딩이 필요하다
		
		// 쿠키가 저장될 경로 설정하기 (생략하면 컨텍스트패스 경로에 저장된다.)
		cookie1.setPath("/servlet");          // 컨텍스트패스 : request.getContextPath()로 알아낼수 있음
		cookie2.setPath("/servlet/cookie1");  // 서블릿경로   : request.getRequestURI()
		                                      // cookie3은 경로 설정을 생략했으므로 컨텍스트패스에 저장된다.
		
		// 쿠키가 유지되는 시간 설정하기 (생략하면 세션쿠키가 된다. 브라우저를 닫으면 지워진다. 시간을 0으로 설정하고 덮어쓰면 쿠키가지워짐)
