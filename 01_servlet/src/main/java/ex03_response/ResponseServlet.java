...
public class ResponseServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * 응답 (Response)
		 * 1. 서버가 클라이언트에게 데이터를 보내는 것을 의미한다.  (서버 -> 클라이언트)
		...
		//    IOException 예외 처리가 필요하지만 doGet() 메소드는 IOException 처리를 이미 하고있다.
		...
		out.println("<ul>");
		out.println("<li>성별: " + gender + "</il>");
		out.println("<li>도메인: " + domain + "</il>");
		out.println("<li><pre>" + content + "</pre></il>");
		out.println("<li>지역: " + region + "</il>");
		out.println("</ul>");
		out.println("</div>");
		out.println("</body>");
		out.println("</html>");
